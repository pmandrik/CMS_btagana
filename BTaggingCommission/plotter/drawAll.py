...
def drawLatexCMS(x=0.10, y=0.91):
...
def drawLatexLumi(x=0.89,y=0.91):
...
def convHStackToHist(hs):
    print type(hs)
    if type(hs) == ROOT.TH1D : return hs
...
def draw(cfg, isLog=False):
    ...
    for dIdx, data in enumerate(cfg['data']):
        ...
        print data['fileIndex']
        print "fIn[data['fileIndex']]"
        #print fIn[data['fileIndex']].ls()

        print " ==== ==== ==== "
        print cfg['name']+data['postfix'][0]
        print fIn[data['fileIndex']].Get(cfg['name']+data['postfix'][0])
        if not fIn[data['fileIndex']].Get(cfg['name']+data['postfix'][0]) : continue
        if fIn[data['fileIndex']].Get(cfg['name']+data['postfix'][0]).GetSumOfWeights() < 0.001 : continue
        ...
        for hIdx, hist in enumerate(data['hist']):
            ...
            if data['isData']:
                hist.SetLineColor  (data['fillColor'][hIdx][0])
                hist.SetMarkerColor(data['fillColor'][hIdx][0])
                hist.SetMarkerStyle(data['fillColor'][hIdx][1])
                hist.SetMarkerSize(1.00)
            else:
                hist.SetLineColor  (data['fillColor'][hIdx])
                hist.SetMarkerColor(data['fillColor'][hIdx])
        ...
        for hIdx, hist in enumerate(data['hist']):
            if 'hTotal' in data.keys():
                ...
                if data['isData']:
                    ...
                    data['hTotal'].SetLineColor  (data['fillColor'][-1][0])
                    data['hTotal'].SetMarkerColor(data['fillColor'][-1][0])
                    data['hTotal'].SetMarkerStyle(data['fillColor'][-1][1])
                else:
    ...
    for dIdx, data in enumerate(cfg['data']):
        print "====>"
        print cfg['data'][1]['hTotal'].GetSumOfWeights()
        print convHStackToHist(data['hTotal']).GetSumOfWeights()
    ...
    stLeg.Draw()
    drawLatexCMS    (x=0.10,y=0.91)
    drawLatexLumi   (x=0.89,y=0.91)
    ...
    for oIdx, rpOpt in enumerate(cfg['ratioOpts']):
        for dIdx, rp in enumerate(cfg['hRatio'][1:],start=1):
            rp[oIdx].SetH1DrawOpt(cfg['data'][0]['drawOpt'])
            rp[oIdx].SetH2DrawOpt(cfg['data'][dIdx]['drawOpt'])
            rp[oIdx].SetGraphDrawOpt("P Z 0 2")
            rp[oIdx].Draw()
            rp[oIdx].GetXaxis().SetTitle(cfg['xTitle'])
            rp[oIdx].GetUpperRefYaxis().SetTitle(cfg['yTitle'])
            rp[oIdx].GetLowerRefYaxis().SetTitle("Ratio")
            rp[oIdx].GetLowerRefYaxis().SetNdivisions(505)
            rp[oIdx].GetLowerRefYaxis().SetLabelSize(0.03)
            rp[oIdx].GetLowerRefGraph().SetMaximum(1.5)
            rp[oIdx].GetLowerRefGraph().SetMinimum(0.5)
            rp[oIdx].GetLowerRefGraph().SetFillColor  (cfg['data'][dIdx]['fillColor'][-1][0])
            rp[oIdx].GetLowerRefGraph().SetLineColor  (cfg['data'][dIdx]['fillColor'][-1][0])
            rp[oIdx].GetLowerRefGraph().SetMarkerColor(cfg['data'][dIdx]['fillColor'][-1][0])
            rp[oIdx].GetLowerRefGraph().SetMarkerStyle(cfg['data'][dIdx]['fillColor'][-1][1])
            if "rpExtra" in cfg.keys():
                exec cfg['rpExtra']
        cfg['hRatio'][1][oIdx].Draw()
        for dIdx, rp in enumerate(cfg['hRatio'][2:],start=2):
            cfg['hRatio'][1][oIdx].GetUpperPad().cd()
            cfg['data'][dIdx]['hTotal'].Draw(cfg['data'][dIdx]['drawOpt']+" SAME")
            cfg['hRatio'][1][oIdx].GetLowerPad().cd()
            rp[oIdx].GetLowerRefGraph().Draw("0 P SAME")
        canvas.cd()
        rpLeg.Draw()
        drawLatexCMS    (x=0.10,y=0.93)
        drawLatexLumi   (x=0.88,y=0.93)
