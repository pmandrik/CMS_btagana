...
# /afs/cern.ch/work/p/pmandrik/swork/2_btag_ttbar/11_UL_2018/BTaggingCommission/plotter/runCode4ttbar.sh flist_ttbar_part00.py /afs/cern.ch/work/p/pmandrik/swork/2_btag_ttbar/7_jan_2018_SF/CMSSW_10_0_0/src/pileupWgts.root
...
def runJob(args):
    ...
    if len(inputFiles) > 0 : 
        ...
        for iTrg in range(1, triggerpaths.GetNbinsX()+1):
            ...
            trigger_name = triggerpaths.GetXaxis().GetBinLabel(iTrg)
            trigger_index = triggers.index( trigger_name ) + 1

            if triggerpaths.GetXaxis().GetBinLabel(iTrg) not in runJobConfig.skipTriggers: # Skip prescaled and non-emu trigger bits
                print( triggerpaths.GetXaxis().GetBinLabel(iTrg) ), trigger_index, " <= "
                run.AddTrigChannel( 11*13 )
            else:
...
def merge(args):
    ...
    for groupKey, groupVal in groupdata.iteritems():
        ...
        for datasetIdx, datasetKey in enumerate(groupVal['dataset']):
            ...
            #hTotalGenEvts.Print()
            nEventsAll = hTotalGenEvts.GetBinContent(1) if groupVal['xsec'][datasetIdx] > 0 else 1.
            ...
            if groupVal['xsec'][datasetIdx] > 0 : 
                    for iBin in range(1, hPuWgtNorms.GetNbinsX()+1, 2):
                        ...
                        # print( hPuWgtNorms.GetBinContent(iBin), hPuWgtNorms.GetXaxis().GetBinLabel(iBin) )
            ...
            for h in range(0, nHistos):
                ...
                if htemp.InheritsFrom('TH1') or htemp.InheritsFrom('TH2'):
                    scale = lumiScale
                    if "ttbar_jet0_pt_nominal" == histoName : 
                      print("scale", scale)
                      print(float(groupVal['xsec'][datasetIdx]*groupVal['lumi']), lumiScale)
                      if nEventsAll < 1 : nEventsAll = 1
                      print( "eff = ", htemp.Integral() / nEventsAll )
                      print( "eff events = ", htemp.Integral() / nEventsAll * float(groupVal['xsec'][datasetIdx]*groupVal['lumi']) )
                    for k,v in puWgtNormScale.iteritems():
                        if k in htemp.GetName():
                            scale *= v
                    if "ttbar_jet0_pt_nominal" == histoName : print("scale", scale)
                    if histoName not in final_histos.keys():
                        ...
                        #print("THIS", scale, histoName)
                    else:
                        final_histos[histoName].Add(htemp, scale) 
                        #print("not THIS", scale, histoName)
